...
enum ZSH {
  ...
  @MainActor static func withColor<T>(_ color: Color, action: () throws -> T) rethrows -> T {
    if Terminal.shouldApplyColors { Terminal.print("%F{\(color.rawValue)}", terminator: "") }
    let result = try action()
    if Terminal.shouldApplyColors { Terminal.print("%f", terminator: "") }
    return result
  }

  @MainActor static func withBold<T>(action: () throws -> T) rethrows -> T {
    if Terminal.shouldApplyStyles { Terminal.print("%B", terminator: "") }
    let result = try action()
    if Terminal.shouldApplyStyles { Terminal.print("%b", terminator: "") }
    return result
  }
