...
enum App {
    ...
    @MainActor
    static func throwError(_ message: Terminal.StyledString) -> Never {
        Terminal.print("""
    ...
    @MainActor
    static func printVirtualEnvSection() {
    ...
    @MainActor
    static func printPathSection(repository: Git.Repository?) {
        ZSH.withColor(.red) {
    ...
    @MainActor
    static func printGitSection(repository: Git.Repository?) {
        guard let repository = repository else {return }
        ZSH.withColor(.blue) { Terminal.print(" git:(", terminator: "") }
        switch repository.reference {
        case let .branch(branch):
            ZSH.withColor(.yellow) { Terminal.print(branch, terminator: "") }
        case let .rebaseHash(rebaseHash):
    ...
    @MainActor
    static func printPrivilegeSection() {
    ...
    @MainActor
    static func printExitCodeSection() {
    ...
    static func main() {
        for argument in CommandLine.arguments.map({ $0.trimmingCharacters(in: .newlines) }).dropFirst() {
            switch argument {
            case "-h", "--help":
                printHelp()
                exit(EXIT_SUCCESS)
            case "-v", "--version":
                ...
                exit(EXIT_SUCCESS)
            default:
                throwError("unrecognized \(argument.isCLIOption ? "option" : "argument") \"\(argument)\" provided.")
            }
        }
        printPrompt()
    }
