...
enum FileSystem {
  static var currentLogicalPath: String {
    ...
    guard let pwd = getenv("PWD") else { return physicalPath }
    let logicalPath = String(cString: pwd)
    return
      if let resolvedLogicalPath = realPath(of: .init(cString: pwd)),
      resolvedLogicalPath == physicalPath
    { logicalPath } else { physicalPath }
  }

  private static func realPath(of relativePath: String) -> String? {
    guard FileManager.default.fileExists(atPath: relativePath) else { return nil }
    let absolutePath = realpath(relativePath, nil)!
    defer { free(absolutePath) }
  ...
  static var canModifyCurrentDirectory: Bool { access(".", W_OK) == 0 }
}
