...
enum Git {
  struct Repository {
    ...
    private static func unsafeParseHead(
      at path: UnsafeMutableBufferPointer<CChar>, sentinelOffset: Int
    ) -> Repository? {
      guard let file = fopen(path.baseAddress!, "r") else { return nil }
      defer { fclose(file) }
      return withUnsafeTemporaryAllocation(of: CChar.self, capacity: 256) { buffer in
        ...
        for (offset, branchMatchingByte) in "ref: re".utf8.enumerated() {
          if !matchesBranch && !matchesRebaseHash { return nil }
          let readByte = fgetc(file)
          guard readByte != EOF else { return nil }
          if readByte != branchMatchingByte { matchesBranch = false }
          if !ASCII.isHEXDigit(readByte) { matchesRebaseHash = false }
          if matchesRebaseHash { buffer[offset] = CChar(readByte) }
        }
        ...
        for branchMatchingByte in "fs/heads/".utf8 {
          let readByte = fgetc(file)
          guard readByte != EOF && readByte == branchMatchingByte else { return nil }
        }
        var offset = 0
        while true {
          ...
          guard readByte != EOF && readByte != ASCII.newlineValue else {
            if offset == 0 { return nil } else { break }
          }
        ...
        return Repository(
          path: String(cString: path.baseAddress!),
          reference: .branch(String(cString: buffer.baseAddress!)))
      }
    }
    ...
    static var active: Repository? {
      ...
      return withUnsafeTemporaryAllocation(
        of: CChar.self, capacity: currentDirectoryPathUTF8.count + 11
      ) { repositoryPath in
          ...
          if isRootDirectory == nil {
            ...
            if !isRootDirectory! {
              ...
              for offset in 0..<currentDirectoryPathUTF8.count {
                if repositoryPath[offset] == ASCII.slashValue { totalSlashes += 1 }
              }
              if totalSlashes > 1 {
                slashOffsets = []
                ...
                while offset > 0 {
                  defer { offset -= 1 }
                  if repositoryPath[offset] == ASCII.slashValue { slashOffsets!.append(offset) }
                }
              }
            }
          }
          if isRootDirectory! { break }
